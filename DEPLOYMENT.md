...
| `GET /api/ide/health` | Compose `healthcheck`. Independent call before `/health`: HTTP 200 `audit_chain_ok: true` with `audit_events: 0` / `audit_hmac: false`. Does **not** lazy-init. `GET /api/world/state` stays HTTP 500. |
