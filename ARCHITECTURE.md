...
**Agent Execution (library path; default tick does not reach it):**
1. `process_agent` returns immediately if `agent.current_task_id` is empty
2. Default-seed assistants have no current task and are not in `office.agents`, so they are never passed to `process_agent`
3. When a registered Task is present, the function calls `task.check_preconditions()`, not `can_handle_task`. It does not execute user code
4. `needs_meeting()` only sets `agent.status = "in_meeting"` and returns. It does not call `hold_meeting()`
5. There is no “request support” implementation beyond setting `blocked`

**Manager Decision (library path; default Alice has no staff):**
1. `process_manager` walks `manager.managed_agents` (default `[]`)
2. Consensus / approval only runs if a managed agent already has a registered Task in APPROVAL
3. Default seed: Alice `managed_agents == []`, registered tasks == 0, so `sim.step()` does not review, vote, approve, or transition work
...
**Intended path (not the default seed):** a user intent becoming assigned, executed, verified, and merged work.

Default `init_simulation()` does not register `task-001`, does not assign work, and does not run that pipeline. Independent `sim.step()` leaves 11 agents `idle`. Metrics **Tasks** stays 0. `GET /api/tasks` is `[]`.
