...
4. **STEP / START / STOP / REFRESH**: those are the simulation button labels (not “STEP (+1 Tick)” or “REFRESH STATE”). START returns `"Simulation started"` without setting `is_running` (test client: tick_count stays 0). The button paints RUNNING locally. STOP sets `world.is_active = False` (terminal).
