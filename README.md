...
- `GET /api` still names the product a “Cognitive IDE”. The `endpoints` dict lists **33** of **74** routes (omits `/api/ide/*` and `/api/canonical-bundle*`). `GET /health` body `"simulation": "running"` means the object exists, not that START is on. `GET /api/canonical-bundle` `is_complete: true` is 27 non-None slots (empty archives count). The report also prints **CIVILIZATION LAYER: FINISHED** plus Legitimate/Auditable/Reproducible/Governed/Bounded/Trustworthy: Yes. Charter `verify_signature` always returns True. Consigliere / Head of Security JSON flags are hardcoded; no UI chrome. `POST /api/consigliere/assess` is a substring check. `POST /api/security/audit` returns `is_complete: false`, `findings: []`. Shipped client loads Socket.IO from `cdn.socket.io`
- `GET /api/world/state` is **HTTP 500** until something else lazy-inits the global (`/health` does; this route does not). Opening `GET /` does not init; the shipped client still calls this on Socket.IO connect. `GET /metrics` is **503** until then; HELP `minioffice_floors_total` says “floors” and counts `len(world.floors)` (**2**), not 28 language floors. Charter HTTP JSON has **no** `digital_signature` field (the hash is only inside `human_readable`)
- `GET /api/floors/<directory>` is **HTTP 404** for `wasm` / `cuda` / `matlab` / `objective-c` / `rust-async`. API enum keys are `webassembly` / `cuda_gpu` / `matlab_octave` / `objective_c` / `rust_async`. `GET /api/floors` is 28 spec dataclasses, not the 2 seeded `World.Floor` objects
