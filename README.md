...
This README reports the **measured** state of parent [`f24ae5c`](https://github.com/IAmSoThirsty/Thirstys-Projects-Miniature-Office/commit/f24ae5c3d419f9bb14388591b05b8fb40ab70cc0) (code pin [`ffd9b5e`](https://github.com/IAmSoThirsty/Thirstys-Projects-Miniature-Office/commit/ffd9b5e7310194c713473941a06eaf797cfdfd38)) plus the CI/docs repair on this pin. Independent pytest: **1,567 passed**, 1 skipped, 12.43s. Evidence: [CLAIMS_AUDIT.md](CLAIMS_AUDIT.md). Score that must sum: [CLAIMS_LEDGER.md](CLAIMS_LEDGER.md). Index: [DOCS.md](DOCS.md).
...
| CI / CD | CI **failed** on `f24ae5c` ([33208378504](https://github.com/IAmSoThirsty/Thirstys-Projects-Miniature-Office/actions/runs/33208378504)): bandit JSON dump without `-ll` exited 1 on 13 lows; Python 3.9 cannot install pytest 9.0.3. CD **succeeded** ([33208378559](https://github.com/IAmSoThirsty/Thirstys-Projects-Miniature-Office/actions/runs/33208378559)). This pin drops 3.9 and makes the JSON dump `--exit-zero`. Actions on *this* SHA have not yet run |
...
| Tests | 1,537 passing | **1,567 passed**, 1 skipped (`def test_` grep = **1,602**) |
...
| Python | 3.9+ | **3.10+** (pytest 9.0.3 requires ≥3.10; the 3.9 badge was false) |
| macOS `start.command` | Documented | **Present** — launches `start.sh` |
| Editor UI | (older docs denied it) | **Present** |
| PWA | Denied in operator docs | **Present** (`manifest.json`, `sw.js`); no WebXR |
| Audit score | 8 hold / 7 partial / 1 inflated / 3 false | **9 / 6 / 1 / 3** of 19 (sums). Docker/compose moved to Holds after CD was observed green |
...
- Docker Compose files that start gunicorn on port 5000 (`docker compose up --build`). No default `SECRET_KEY`. Production refuses placeholders. CD health-check green on `f24ae5c`
- GitHub Actions unit-test + security jobs (bandit `-ll`, `pip-audit`). The JSON dump no longer fails the job
...
- Bandit still reports 13 **low** findings (the `-ll` gate ignores them on purpose)
- There is no WebXR
...
Requires **Python 3.10+**. Open `http://127.0.0.1:5000`.
