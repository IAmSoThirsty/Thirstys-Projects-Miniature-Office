...
- `POST /api/world/start` returns `success: true` / `"Simulation started"` without setting `is_running`. Independent Flask test client: `is_running` stays **false**, `tick_count` stays **0**. The handler only queues `simulation.run`. Shipped START paints RUNNING from local JS. Alice `managed_agents` is `[]`. Supply-store versions are labels (Interpreter `3.11.0` / PyTest `7.4.0`), not the runtime (`python3 --version` 3.10.21; `pytest==9.0.3`). Compose `GET /api/ide/health` is `audit_chain_ok: true` with **0** events and does not init the world. `GET /api/contracts` is `[]`
