...
- `POST /api/consigliere/preview` of `"freeze building"` returns canned keyword text (`"All execution stops immediately"`, `"No new tasks can start"`). That is not a model of the tree. After `POST /api/security/lockdown` `scope=building` (`is_active: true`), `GET /api/canonical-bundle/freeze-protocol` stays `is_frozen: false`, `POST /api/world/step` still ticks, `POST /api/world/start` still returns `success: true`, and `POST /api/ide/terminal` `echo hello` still exits 0. `GET /api/agents/mgr-001` (no `/status`) is Flask 404 HTML; the shipped route is `GET /api/agents/<id>/status`
