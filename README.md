...
- `POST /api/security/grant` returns `granted: true` and `GET /api/security/permissions` lists the grant (`active_permissions: 1`), but `GET /api/canonical-bundle/authority-ledger` stays `total_grants: 0`. `POST /api/security/lockdown` `scope=building` is `is_active: true` (`active_lockdowns: 1`) while `GET /api/canonical-bundle/freeze-protocol` stays `is_frozen: false` and `POST /api/world/step` still succeeds. `POST /api/security/block` of `task-001` is `is_blocked: true`; `security-dossiers` / `override-ledger` stay 0
