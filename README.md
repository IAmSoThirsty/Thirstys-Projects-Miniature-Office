...
- `GET /api/agents` before `/health` is **HTTP 200** `{"agents": []}` (same empty-200 for departments / supply-store / audit/events / tasks). `GET /api/floors` is **28** `FloorSpecification` dataclasses, not `world.floors`. HELP `minioffice_audit_events_total` says “Total” but omits `DIRECTIVE_CREATED` (**42** vs `GET /api/audit/events` **43**)
