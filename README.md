...
- `GET /api` still names the product a “Cognitive IDE”. `GET /health` body `"simulation": "running"` means the object exists, not that START is on. `GET /api/canonical-bundle` `is_complete: true` is 27 non-None slots (empty archives count) and is HTTP 200 **before** the simulation exists. Charter `verify_signature` always returns True. Bundle sub-ledgers are a parallel empty store: `tool-provenance` `total_tools: 0` while the supply store has **2** tools; `floor-profiles` `total_profiles: 0` while `GET /api/floors` returns **28** specs; `simulation-traces` stay 0 after STEP; `formal-verification` is 0/0. Consigliere / Head of Security JSON flags are hardcoded; no UI chrome. Correct-shaped Consigliere POSTs return `success: true` without changing agent status or writing `consigliere-logs`. `POST /api/security/audit` increments `audits` with `is_complete: false` and does not write `security-dossiers`. Shipped client loads Socket.IO from `cdn.socket.io`
