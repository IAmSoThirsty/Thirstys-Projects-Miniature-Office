...
| Security grant writes the authority ledger | No. Correct-shaped `POST /api/security/grant` returns `granted: true` and `GET /api/security/permissions` lists it (`active_permissions: 1`). `GET /api/canonical-bundle/authority-ledger` stays `total_grants: 0` / `active_grants: 0`. |
| Building lockdown freezes the world / freeze-protocol | No. `POST /api/security/lockdown` `scope=building` returns `is_active: true` (`GET /api/security` `active_lockdowns: 1`). `GET /api/canonical-bundle/freeze-protocol` stays `is_frozen: false` / `access_locked: false`. `POST /api/world/step` still returns `success: true`. |
| Security block writes security-dossiers / override-ledger | No. `POST /api/security/block` of `task-001` returns `is_blocked: true`. `GET /api/canonical-bundle/security-dossiers` stays `total_decisions: 0`. `override-ledger` stays `total_overrides: 0`. |
