...
| Canonical bundle `is_complete` / charter `is_immutable` / purpose-lock LOCKED | `verify_bundle_completeness()` checks 27 slots are not `None`. Empty archives still count. The bundle route is HTTP 200 **before** the simulation exists. Charter signature is `sha256(b"charter-001")`; `verify_signature` always returns True. Purpose lock `overall_locked=True` with **0** subsystems checked. Authority ledger grants **0**. Sub-ledgers are unwired: `tool-provenance` 0 vs supply-store 2; `floor-profiles` 0 vs 28 specs; traces 0 after STEP; `formal-verification` 0/0. |
| Consigliere / Head of Security JSON is the running executive layer | `can_alter_execution` / `can_force_rearchitecture` are hardcoded `True`. `src/client/index.html` never calls those routes. The tick does not import them. Correct-shaped Consigliere POSTs return `success: true` without changing agent status or writing `consigliere-logs`. `POST /api/security/audit` increments `audits` with `is_complete: false` and does not write `security-dossiers`. |
