...
| Consigliere preview of freeze stops execution | No. `POST /api/consigliere/preview` with `"action": "freeze building"` is a `"freeze" in action.lower()` keyword table. Consequences include “All execution stops immediately” and “No new tasks can start”. After a real building lockdown (`is_active: true`), freeze-protocol stays `is_frozen: false`, STEP still ticks, START still returns `success: true`, and `POST /api/ide/terminal` still runs. |
| `GET /api/agents/<id>` is the agent | No. That path is Flask 404 HTML. The shipped route is `GET /api/agents/<id>/status`. |
