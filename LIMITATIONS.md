...
| `GET /health` `"simulation": "running"` means START | No. It means the global `simulation` object exists (the handler lazy-inits it). Independent `GET /api/world/state` `"is_running"` is **false** until `POST /api/world/start` **and** the Socket.IO background task actually sets the flag. HTTP status is liveness 200. |
| STOP pauses the loop so START / STEP can resume | No. `SimulationEngine.stop()` sets `world.is_active = False`. `tick()` returns immediately. START does not set `is_active` True. STEP after STOP returns `success: true` with unchanged `tick_count`. `shutdown-protocol` stays `is_shutdown: false`. |
| START is a continuous loop the UI can stop | No. On `python3 run.py`, `POST /api/world/start` returns `success: true` then wedges HTTP: `GET /health`, `GET /api/world/state`, and `POST /api/world/stop` time out. The shipped START button paints RUNNING. |
