...
**Status: experimental prototype.** Operator-facing limitation list. Evidence: [CLAIMS_AUDIT.md](CLAIMS_AUDIT.md). Measured 28 August 2026.
...
- Docker Compose / GitHub Actions **files**; CI and CD are green on `b8827e7`
- **1,541 tests passing**, 1 skipped (local pytest; prior GitHub Actions CI green)
...
| Durable audit / hash chain | Memory dict with `prev_hash` + parent hashes. Not signed. Not persisted. |
| Security CI as a gate | `safety` and `bandit` cannot fail the job (`\|\| true`). |
| CD | Green on `b8827e7` ([run 33174907349](https://github.com/IAmSoThirsty/Thirstys-Projects-Miniature-Office/actions/runs/33174907349)). Uses `docker compose`. Default `SECRET_KEY` is still a placeholder. |
...
| --- | --- | --- |
| Local + GitHub pytest | 1,541 passed, 1 skipped (this pass, local 3.10) | Collected tests |
